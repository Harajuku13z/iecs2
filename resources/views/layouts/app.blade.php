        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    @php
                        // Pages statiques dans le menu (après Accueil)
                        $staticPagesMenu = \App\Models\StaticPage::menuVisible()->mainMenu()->orderBy('menu_ordre')->get();
                        $staticPagesSubMenus = \App\Models\StaticPage::menuVisible()->whereNotNull('menu_parent')->where('menu_parent', '!=', '')->orderBy('menu_ordre')->get()->groupBy('menu_parent');
                    @endphp
                    @foreach($staticPagesMenu as $staticPage)
                        @php
                            $hasSubMenu = $staticPagesSubMenus->has($staticPage->menu_nom);
                            $subMenuItems = $hasSubMenu ? $staticPagesSubMenus->get($staticPage->menu_nom) : collect();
                        @endphp
                        @if($hasSubMenu)
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle {{ request()->is('page/' . $staticPage->slug) || request()->is('page/' . $staticPage->slug . '*') ? 'active' : '' }}" href="#" role="button" data-bs-toggle="dropdown">
                                    {{ $staticPage->menu_nom }}
                                </a>
                                <ul class="dropdown-menu">
                                    <li>
                                        <a class="dropdown-item {{ request()->is('page/' . $staticPage->slug) ? 'active' : '' }}" href="{{ route('static-page.show', $staticPage->slug) }}">
                                            {{ $staticPage->menu_nom }}
                                        </a>
                                    </li>
                                    ...
                                    @foreach($subMenuItems as $subItem)
                                        <li>
                                            <a class="dropdown-item {{ request()->is('page/' . $subItem->slug) ? 'active' : '' }}" href="{{ route('static-page.show', $subItem->slug) }}">
                                                {{ $subItem->menu_nom }}
                                            </a>
                                        </li>
                                    @endforeach
                        ...
                        @else
                            <li class="nav-item">
                                <a class="nav-link {{ request()->is('page/' . $staticPage->slug) ? 'active' : '' }}" href="{{ route('static-page.show', $staticPage->slug) }}">
                                    {{ $staticPage->menu_nom }}
                                </a>
                            </li>
                        @endif
                    ...
                    <!-- Menu standard après les pages statiques -->
                    <li class="nav-item">
